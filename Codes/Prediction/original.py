...
class stem_block(nn.Module):
    ...
    def forward(self,x):
        ...
        x = torch.concat([func(x) for func in self.model],dim=-2)
...
class MHA_block(nn.Module):
    ...
    def layer_norm(self,x,device,eps=1e-6):
        a_2=nn.Parameter (torch.ones(x.size(-1))).to(device)
        b_2=nn.Parameter (torch.zeros(x.size(-1))).to(device)
        mean = x.mean(-1, keepdim=True)# mean: [bsz, max_len, 1]
        std = x.std(-1, keepdim=True)
        return a_2 * (x - mean) / (std + eps) + b_2
        
    def rms_norm(self,x,device,eps=1e-6):
        weight = nn.Parameter(torch.ones(x.size(-1))).to(device)
    ...
    def forward(self,x ,mask,norm):
        ...
        if norm == "layer_norm":
            x = self.layer_norm(x,self.device)
        elif norm =="rms_norm":
            x = self.rms_norm(x,self.device)
        else:
            x = x
